...
def get_raw_transcripts(link: str, speech_id: int) -> List[List[str]]:
    ...
    for item in raw_text:
        ...
        item = re.sub(triangle_braces, "", item)
...
def download_data():
    with open("../raw_data/transcript_links.txt", "r") as links_file:
        links = [line.strip() for line in links_file]

    speech_id = 0
    with open("../raw_data/raw_transcripts.tsv", "w") as out_f:

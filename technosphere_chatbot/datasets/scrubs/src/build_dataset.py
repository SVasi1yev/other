...
def build_dataset(character="J.D.", context_length=6):
    data = []
    with open("../raw_data/filtered_transcripts.tsv", "r") as filtered_transcripts_file:
        scene = []
...
def save_dataset(dataset, character, context_length):
    smooth_name = re.sub(r" ", "_", character)
    smooth_name = re.sub(r"\.", "", smooth_name)
    with open("../dataset_context_{}_{}.tsv".format(context_length, smooth_name), "w") as out_f:

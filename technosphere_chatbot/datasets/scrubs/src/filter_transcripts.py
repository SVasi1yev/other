...
def get_all_characters():
    MAX_LENGTH = 80
    characters = set()
    with open("../raw_data/raw_transcripts.tsv", "r") as transcripts_file:
        for line in transcripts_file:
            character = line.strip().split("\t")[1]
            if len(character[1]) < MAX_LENGTH:
                characters.add(character)

    with open("../raw_data/all_characters.txt", "w") as characters_file:
        for character in characters:
            characters_file.write("{}\n".format(character))


def filter_transcripts():
    with open("../raw_data/verified_characters.txt", "r") as characters_file:
        verified_characters = {character.strip() for character in characters_file}

    transcripts = [[0, scene_delimiter, scene_delimiter], ]
    with open("../raw_data/raw_transcripts.tsv", "r") as raw_transcripts_file:
    ...
    with open("../raw_data/filtered_transcripts.tsv", "w") as filtered_transcripts_file:
